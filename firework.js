...
.then((text) => {
  ...
  parts.forEach((element) => {
    ...
    var splitted_element = element.split("---");
    
    // Delete newlines and add resultimg Object to an array:
    riddles.push({question: splitted_element[0].replace(/[\r\n]/gm, ''),
                      answer: splitted_element[1].replace(/[\r\n]/gm, '')});
  })
})
.catch((e) => console.error(e));


// Функция для добавления красивого стиля к вопросу
function addStyleToQuestions() {
  var questionElements = document.getElementsByClassName("question");

  for (var i = 0; i < questionElements.length; i++) {
    questionElements[i].classList.add("bold");
  }
}
...
function checkAnswer() {
  ...
  if (answerInput === currentRiddle.answer.toLowerCase()) {
    currentRiddleIndex++;
    if (currentRiddleIndex === riddles.length - 1) {
      a.run();
      document.getElementById("riddle").innerHTML =
        riddles[currentRiddleIndex].question;
      //window.location.href = currentRiddle.nextPage;
    } else {
      document.getElementById("riddle").innerHTML =
        riddles[currentRiddleIndex].question;
    ...
    alert("Ключ введен неправильно. Попробуй еще раз.");
  }
}
